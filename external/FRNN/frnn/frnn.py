...
class _frnn_grid_points(Function):
    ...
    def forward(
        ...
        """
        use_cached_grid = (
            sorted_points2 is not None
            and pc2_grid_off is not None
            and sorted_points2_idxs is not None
        ...
        if not use_cached_grid:
            ...
            grid_params_cuda = torch.zeros(
                (N, grid_params_size), dtype=torch.float, device=points1.device
            )
            G = -1
            for i in range(N):
                ...
                grid_params_cuda[i, grid_delta_idx + 1 : grid_total_idx] = (
                    torch.floor(grid_size / cell_size) + 1
                )
            ...
            pc2_grid_cell = torch.full(
                (N, P2), -1, dtype=torch.int, device=points1.device
            )
            pc2_grid_idx = torch.full(
                (N, P2), -1, dtype=torch.int, device=points1.device
            )
            _C.insert_points_cuda(
                points2,
                lengths2,
                grid_params_cuda,
                pc2_grid_cnt,
        ...
        for i in range(N):
            prefix_sum_cuda(
                pc1_grid_cnt[i], grid_params[i, grid_total_idx], pc1_grid_off[i]
            )

        sorted_points1 = torch.zeros(
            (N, P1, D), dtype=torch.float, device=points1.device
        )
        sorted_points1_idxs = torch.full(
            (N, P1), -1, dtype=torch.int, device=points1.device
        )
        _C.counting_sort_cuda(
            points1,
            lengths1,
...
def frnn_grid_points(
    ...
    if lengths1 is None:
        lengths1 = torch.full(
            (points1.shape[0],), P1, dtype=torch.long, device=points1.device
        )
    if lengths2 is None:
        lengths2 = torch.full(
            (points2.shape[0],), P2, dtype=torch.long, device=points2.device
        )
...
def frnn_gather(
    x: torch.Tensor, idxs: torch.Tensor, lengths: Union[torch.Tensor, None] = None
):
    """
...
class _frnn_bf_points(Function):
    ...
    def forward(ctx, points1, points2, lengths1, lengths2, K, r):
        idxs, dists = _C.frnn_bf_cuda(points1, points2, lengths1, lengths2, K, r)
...
def frnn_bf_points(points1, points2, lengths1, lengths2, K, r, return_nn=False):
    idxs, dists = _frnn_bf_points.apply(points1, points2, lengths1, lengths2, K, r)
    points2_nn = None
    if return_nn:
        points2_nn = frnn_gather(points2, idxs, lengths2)
