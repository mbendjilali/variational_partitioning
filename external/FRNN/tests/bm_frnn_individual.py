...
def bm_frnn_prefix_sum(fnames):
    ...
    outputs = benchmark(
        TestFRNN.frnn_prefix_sum, "prefix_sum", kwargs_list, num_iters=5, warmup_iters=1
    )
    return outputs
...
def bm_frnn_find_nbrs(fnames):
    ...
    outputs = benchmark(
        TestFRNN.frnn_find_nbrs, "find_nbrs", kwargs_list, num_iters=5, warmup_iters=1
    )
    return outputs
