...
def save_intermediate_results(fnames):
    ...
    for fname in fnames:
        ...
        pc1 = torch.FloatTensor(
            read_ply(fname)[None, :, :3]
        ).cuda()  # no need for normals
        ...
        dists, idxs, nn, grid = frnn.frnn_grid_points(
            pc1, pc2, lengths1, lengths2, K=k, r=0.1, filename=fname.split("/")[-1]
        )
        print(fname + " done!")
...
class TestFRNN(unittest.TestCase):
    ...
    def frnn_setup_grid(N, fname):
        ...
        if ragged:
            ...
            lengths2 = (
                torch.ones((N,), dtype=torch.long, device=points2.device)
                * points2.shape[1]
            )
        GRID_PARAMS_SIZE = 8
        grid_params_cuda = torch.zeros(
            (N, GRID_PARAMS_SIZE), dtype=torch.float, device=points2.device
        )
        ...
        def output():
            ...
            for i in range(N):
                ...
                grid_params_cuda[i, 7] = (
                    grid_params_cuda[i, 4]
                    * grid_params_cuda[i, 5]
                    * grid_params_cuda[i, 6]
                )
    ...
    def frnn_insert_points(N, fname):
        ...
        if ragged:
            ...
            lengths1 = (
                torch.ones((N,), dtype=torch.long, device=points1.device)
                * points1.shape[1]
            )
            lengths2 = (
                torch.ones((N,), dtype=torch.long, device=points2.device)
    ...
    def frnn_counting_sort(N, fname):
        ...
        sorted_points1 = torch.zeros(
            (N, P1, 3), dtype=torch.float, device=points1.device
        )
        sorted_points1_idxs = torch.full(
            (N, P1), -1, dtype=torch.int, device=points1.device
        )
        sorted_points2 = torch.zeros(
            (N, P2, 3), dtype=torch.float, device=points1.device
        )
    ...
    def frnn_find_nbrs(N, fname, K):
        ...
        if ragged:
            ...
            lengths1 = (
                torch.ones((N,), dtype=torch.long, device=sorted_points1.device)
                * sorted_points1.shape[1]
            )
            lengths2 = (
                torch.ones((N,), dtype=torch.long, device=sorted_points2.device)
                * sorted_points2.shape[1]
            )
        torch.cuda.synchronize()
