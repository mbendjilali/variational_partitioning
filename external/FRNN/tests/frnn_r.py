...
def frnn_r(fname, N, K, r):
    ...
    if ragged:
        ...
        lengths1 = (
            torch.ones((N,), dtype=torch.long, device=points1.device) * points1.shape[1]
        )
        lengths2 = (
            torch.ones((N,), dtype=torch.long, device=points2.device) * points2.shape[1]
        )
