...
class TestFRNN:
    def __init__(self, num_pcs=1, K=5, r=1.0, D=4, backward=False):
        pc1 = torch.rand((num_pcs, num_points_fixed_query, D), dtype=torch.float)
        pc2 = torch.rand((num_pcs, num_points_fixed_query, D), dtype=torch.float)
        ...
        if backward:
            ...
            self.grad_dists_knn = torch.ones(
                (num_pcs, pc1.shape[0], K), dtype=torch.float32
            ).cuda()
    ...
    def frnn_bf(self):
        idxs, dists, nn = frnn.frnn_bf_points(
            self.pc1_frnn_bf,
            self.pc2_frnn_bf,
            self.lengths1_cuda,
            self.lengths2_cuda,
    ...
    def compare_frnn_knn(self):
        ...
        if self.backward:
            ...
            loss_frnn_reuse = (dists_frnn_reuse * self.grad_dists_frnn_reuse).sum()
        ...
        diff_keys_percentage_reuse = (
            torch.sum(idxs_frnn_reuse == idxs_frnn_bf).type(torch.float).item()
            / self.K
            / self.pc1_frnn.shape[1]
            / self.num_pcs
        )
        ...
        if self.backward:
            pc1_grad_all_close = torch.allclose(
                self.pc1_frnn.grad, self.pc1_knn.grad, atol=5e-6
            )
            pc1_grad_all_close_reuse = torch.allclose(
                self.pc1_frnn_reuse.grad, self.pc1_knn.grad
            )
            # pc1_grad_all_close_reuse = True
            pc2_grad_all_close = torch.allclose(
                self.pc2_frnn.grad, self.pc2_knn.grad, atol=5e-6
            )
            pc2_grad_all_close_reuse = torch.allclose(
                self.pc2_frnn_reuse.grad, self.pc2_knn.grad
            )
            # pc2_grad_all_close_reuse = True
            return [
                idxs_all_same,
                f"{diff_keys_percentage:.8f}",
                dists_all_close,
                pc1_grad_all_close,
                pc2_grad_all_close,
                idxs_all_same_reuse,
                f"{diff_keys_percentage_reuse:.8f}",
                dists_all_close_reuse,
                pc1_grad_all_close_reuse,
                pc2_grad_all_close_reuse,
            ]
        else:
            return [
                idxs_all_same,
                ...
                dists_all_close,
                idxs_all_same_reuse,
                f"{diff_keys_percentage_reuse:.16f}",
                dists_all_close_reuse,
            ]
