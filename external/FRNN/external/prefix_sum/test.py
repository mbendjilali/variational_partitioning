...
def test(num_pcs, max_num_grids):
    ...
    grid_cnt = torch.randint(
        low=0, high=1000, size=(num_pcs, max_num_grids), dtype=torch.int, device=cpu
    )
    grid_off = torch.full(
        size=grid_cnt.shape, fill_value=-1, dtype=torch.int, device=cpu
    )
    ...
    grid_off_cuda = torch.full(
        size=grid_cnt.shape, fill_value=-1, dtype=torch.int, device=gpu
    )
